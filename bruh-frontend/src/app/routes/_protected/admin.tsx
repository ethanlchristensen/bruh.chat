...
function AdminPage() {
  ...
  const resetQuotaMutation = useMutation({
    mutationFn: (userId: number) =>
      api.post<User>(`/users/${userId}/quota/reset`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "approved-users"] });
  ...
  return (
    <div className="container mx-auto py-8 max-w-6xl px-4 flex flex-col gap-6 h-full">
      <div>
        ...
        <p className="text-muted-foreground">
          Manage users, quotas, and approvals.
        </p>
      </div>
        ...
        <TabsContent value="users" className="flex-1 min-h-0 mt-4">
          <Card className="h-full flex flex-col">
            <CardHeader>
              ...
              <CardDescription>
                Manage daily limits, flow caps, and view usage. Click any limit
                value to edit it inline.
              </CardDescription>
            </CardHeader>
                  ...
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 bg-card z-10">
                      <tr className="border-b text-muted-foreground">
                        ...
                        <th className="text-left py-3 pl-4 font-medium">
                          Actions
                        </th>
                      </tr>
                    </thead>
                      ...
                      {approvedUsers.map((u) => (
                        <UserRow
                          ...
                          isResetting={
                            ...
                            resetQuotaMutation.variables === u.id
                          }
                          ...
                          isUpdating={
                            ...
                            (updateQuotaMutation.variables as any)?.userId ===
                              u.id
                          }
                        />
                      ))}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            onClick={() => {
                              if (
                                confirm(
                                  `Reject and permanently delete user "${u.username}"?`,
                                )
                              ) {
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div className="cursor-default">
              ...
              <p className="text-xs text-muted-foreground">
                {allTimeChat.toLocaleString()} chat ·{" "}
                {allTimeFlow.toLocaleString()} flow
              </p>
            </div>
          </TooltipTrigger>
          <TooltipContent side="top">
            All-time: {allTimeChat.toLocaleString()} from chat,{" "}
            {allTimeFlow.toLocaleString()} from flows
          </TooltipContent>
        </Tooltip>
      </td>
